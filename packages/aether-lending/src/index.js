...
class LendingModule {
    id = "aether-lending";
    name = "Aether Lending Guard";
    description = "Monitors liquidation events and account health across Kamino and Save.";
    PROTOCOLS = {
    ...
    extendServer(app) {
        ...
        app.get('/api/lending/liquidations', async (req, res) => {
        ...
        app.get('/api/lending/protocol/:id', async (req, res) => {
            const { db } = require('../../aether-core/src/db/client');
            try {
                const limit = Math.min(parseInt(req.query.limit) || 50, 500);
                const offset = parseInt(req.query.offset) || 0;
